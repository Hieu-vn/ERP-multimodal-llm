...
async def test_rag_pipeline_initialization(mock_rag_config):
    with (
        patch('erp_ai_core.rag_pipeline.RAGConfig', return_value=mock_rag_config),
        patch('erp_ai_core.rag_pipeline.SentenceTransformerEmbeddings'),
        patch('erp_ai_core.rag_pipeline.Chroma'),
        patch('erp_ai_core.rag_pipeline.AutoTokenizer'),
        patch('erp_ai_core.rag_pipeline.AutoModelForSeq2SeqLM'),
        patch('erp_ai_core.rag_pipeline.AutoModelForCausalLM'),
        patch('erp_ai_core.rag_pipeline.PeftModel'),
        patch('erp_ai_core.rag_pipeline.pipeline'),
        patch('erp_ai_core.rag_pipeline.HuggingFacePipeline'),
        patch('erp_ai_core.rag_pipeline.VectorSearchTool'),
        patch('erp_ai_core.rag_pipeline.LiveERP_APITool'),
        patch('erp_ai_core.rag_pipeline.DataAnalysisTool'),
    ):
